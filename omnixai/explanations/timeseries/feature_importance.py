...
class FeatureImportance(ExplanationBase):
    ...
    def plot(
        self, index=None, figure_type=None, max_num_variables_to_plot=25, **kwargs
    ):
        """
        ...
        if len(self.explanations) == 0:
            ...
            print(
                "   2. Training data too short - ensure training data length > test data length"
            )
            print("   3. SHAP parameters - try increasing 'nsamples' parameter")
            print(
                "   4. Model prediction errors - verify detector works with your data format"
            )
            return None
        if figure_type is not None:
            assert figure_type in [
                "timeseries",
                "bar",
        ...
        if figure_type == "timeseries":
            for exp in explanations:
                ...
                for i in range(n):
                    ...
                    right_ax.plot(
                        timestamps, ts_b.values.flatten(), color="r", label="score"
                    )
    ...
    def _plotly_figure(self, index, **kwargs):
        ...
        fig = make_subplots(
            figure=go.Figure(layout=layout), specs=[[{"secondary_y": True}]]
        )
    ...
    def to_json(self):
        raise RuntimeError(
            "`FeatureImportance` for timeseries cannot be converted into JSON format."
        )

    @classmethod
    def from_dict(cls, d):
        raise RuntimeError(
            "`FeatureImportance` for timeseries does not support `from_dict`."
        )
