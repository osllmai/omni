...
class CFExplanation(ExplanationBase):
    ...
    def plot(self, index=None, max_num_variables_to_plot=25, **kwargs):
        ...
        if len(self.explanations) == 0:
            print("⚠️  No explanations to plot! Possible causes:")
            print(
                "   1. Predict function interface issues - check if detector accepts Timeseries objects"
            )
            print("   2. Threshold too high/low - adjust the threshold parameter")
            print(
                "   3. MACE optimization failed - try increasing num_iterations or adjusting learning_rate"
            )
            print(
                "   4. Training data too short - ensure training data length > test data length"
            )
            print(
                "   5. No counterfactuals found - try relaxing optimization parameters"
            )
            return None

        # Check if counterfactuals are None
        has_valid_cf = False
        for exp in self.explanations:
            if exp.get("counterfactual") is not None:
                has_valid_cf = True
                break

        if not has_valid_cf:
            print("⚠️  All counterfactuals are None! Try:")
            print("   1. Lower the threshold (try threshold * 0.1)")
            print("   2. Increase num_iterations (try 200-500)")
            print("   3. Adjust learning_rate (try 0.01-0.1)")
            print("   4. Increase batch_size (try 100-200)")
        figures = []
        explanations = (
            [self.explanations[index]] if index is not None else self.explanations
        )
        ...
        for exp in explanations:
            ...
            if num_variables > max_num_variables_to_plot:
                warnings.warn(
                    "The number of variables in the time series exceeds "
            ...
            for i in range(n):
                ...
                if cf is not None:
                    ...
                    right_ax.plot(
                        timestamps, ts_b.values.flatten(), color="r", label="cf"
                    )
    ...
    def plotly_plot(self, index=0, **kwargs):
        ...
        assert index is not None, (
            "`index` cannot be None for `plotly_plot`. "
            "Please specify the instance index."
        )
    ...
    def ipython_plot(self, index=0, **kwargs):
        ...
        assert index is not None, (
            "`index` cannot be None for `ipython_plot`. "
            "Please specify the instance index."
        )
    ...
    def to_json(self):
        raise RuntimeError(
            "`CFExplanation` for timeseries cannot be converted into JSON format."
        )

    @classmethod
    def from_dict(cls, d):
        raise RuntimeError(
            "`CFExplanation` for timeseries does not support `from_dict`."
        )
