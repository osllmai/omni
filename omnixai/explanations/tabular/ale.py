...
class ALEExplanation(ExplanationBase):
    ...
    def plot(self, class_names=None, **kwargs):
        ...
        for i, feature in enumerate(features):
            ...
            plt.ylabel("Accumulated local effects", fontsize=12)
    ...
    def _plotly_figure(self, class_names=None, **kwargs):
        ...
        fig = make_subplots(
            ...
            x_title="Feature Values",
            y_title="Accumulated Local Effects",
        )
        for i, feature in enumerate(features):
        ...
        fig.update_layout(
            height=fig_height,
            width=fig_width,
            title_text="Accumulated Local Effects (ALE) Analysis",
            title_x=0.5,
            title_font_size=18,
            showlegend=True,
            legend=dict(
            ...
            font=dict(size=12),  # Larger base font size for better readability
        )

        # Update subplot titles to be more readable
        for i in range(len(features)):
            row, col = divmod(i, num_cols)
            fig.update_xaxes(title_text="Feature Values", row=row + 1, col=col + 1)
            fig.update_yaxes(title_text="ALE", row=row + 1, col=col + 1)
