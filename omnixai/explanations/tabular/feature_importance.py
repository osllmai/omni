...
class FeatureImportance(ExplanationBase):
        ...
        for i, index in enumerate(indices):
            ...
            feat_scores = sorted(
                list(
                    zip(
                        [
                            f"{self._s(f)} = {self._s(v)}    "
                            for f, v in zip(exp["features"], exp["values"])
                        ],
                        exp["scores"],
                    )
                ),
            ...
            if self.mode == "classification":
                class_name = (
                    ...
                    else class_names[exp["target_label"]]
                )
    ...
    def plotly_plot(self, index=0, class_names=None, num_features=20, **kwargs):
        ...
        return DashFigure(
            self._plotly_figure(
                index, class_names=class_names, num_features=num_features, **kwargs
            )
        )
    ...
    def plot(self, num_features=20, truncate_long_features=True, **kwargs):
        ...
        feat_scores = sorted(
            list(
                zip(
                    [
                        f"{self._s(f) if truncate_long_features else f}    "
