    ...
    def to_html_div(self, id=None):
        ...
        if isinstance(self.component, (tuple, list)):
            ...
            if height is None or height <= 450:
                return html.Div(
                    [dcc.Graph(figure=self.component, id=id)], id=f"div_{id}"
                )
            else:
                return html.Div(
            ...
            raise ValueError(
                f"The type of `component` ({type(self.component)}) "
                f""
                f"is not supported by DashFigure."
            )
...
class PredictedResults(ExplanationBase):
    ...
    def _plotly_figure(self, index, class_names=None, **kwargs):
        ...
        if labels is None:
            ...
            fnames = (
                [class_names[f] for f in fnames]
                if class_names is not None
                else [str(f) for f in fnames]
            )
        fig = px.bar(
            y=fnames[::-1],
            x=scores[::-1],
