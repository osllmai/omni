...
class Polyjuice(ExplainerBase):
    ...
    def __init__(
        self, predict_function: Callable, mode: str = "classification", **kwargs
    ):
        """
        ...
        from polyjuice_nlp import Polyjuice
    ...
    def _perturb(self, text, **kwargs):
        ...
        if ce_type == "perturb":
            ...
            raise ValueError(
                f"Unknown `ce_type`: {ce_type}. Please choose 'perturb' or 'blank'."
            )
    ...
    def _explain_classification(self, X: Text, max_number_examples: int = 5, **kwargs):
        ...
        for idx, text in enumerate(X.values):
            ...
            examples = sorted(flips, key=lambda x: x[1]) + sorted(
                non_flips, key=lambda x: x[1]
            )
            explanations.add(
                query=pd.DataFrame([[text, original_label]], columns=["text", "label"]),
                cfs=pd.DataFrame(
                    [(e[0], e[2]) for e in examples[:max_number_examples]],
                    columns=["text", "label"],
                ),
            )
        return explanations

    def _explain_question_answering(
        self, X: Text, max_number_examples: int = 5, **kwargs
    ):
        ...
        for x in X:
            ...
            for perturbation in perturb_questions:
                if perturbation == question:
                    continue
                ce_res = self.predict_function(
                    Text(f"{context}{sep}{perturbation}", tokenizer=tokenizer)
                )
                if not isinstance(ce_res, str):
                    ce_res = ce_res[0]
                # Sort the results by a simple distance function
                distance = self._distance(
                    Text(question, tokenizer=tokenizer),
                    Text(perturbation, tokenizer=tokenizer),
                )
                if ce_res == res:
            ...
            examples = sorted(flips, key=lambda z: z[-1]) + sorted(
                non_flips, key=lambda z: z[-1]
            )
    ...
    def explain(self, X: Text, max_number_examples: int = 5, **kwargs) -> CFExplanation:
        ...
        if self.mode == "classification":
            return self._explain_classification(
                X=X, max_number_examples=max_number_examples, **kwargs
            )
        else:
            return self._explain_question_answering(
                X=X, max_number_examples=max_number_examples, **kwargs
            )
