...
class ALE(TabularExplainer):
    ...
    def __init__(
        self, training_data: Tabular, predict_function, mode="classification", **kwargs
    ):
        """
    ...
    def _ale_continuous(self, data, column, bins):
        ...
        delta_cols = OrderedDict(
            {f"delta_{i}": yb[:, i] - ya[:, i] for i in range(ya.shape[1])}
        )
        ...
        for col in delta_cols.keys():
            z = (df[(col, "mean")] + df[(col, "mean")].shift(1, fill_value=0.0)) * 0.5
    ...
    def _categorical_order(self, column, num_bins=100):
        ...
        scores = pd.DataFrame(0.0, index=features, columns=features, dtype=float)
        for i in range(len(self.feature_columns)):
            if i == column:
                continue
            s = pd.DataFrame(0.0, index=features, columns=features, dtype=float)
    ...
    def _ale_categorical(self, data, features, column):
        ...
        z = data.copy()
        z[:, column] = [
            features[min(feature_indices[f] + 1, len(features) - 1)]
        ...
        delta_cols = OrderedDict(
            {f"delta_{i}": ya[:, i] - y[:, i] for i in range(ya.shape[1])}
        )
        cols.update(delta_cols)
        df_a = pd.DataFrame(cols)

        z[:, column] = [
            features[max(feature_indices[f] - 1, 0)] for f in data[:, column]
        ]
        ...
        delta_cols = OrderedDict(
            {f"delta_{i}": y[:, i] - yb[:, i] for i in range(yb.shape[1])}
        )
        cols.update(delta_cols)
    ...
    def explain(
        ...
        **kwargs,
        ...
        if len(feature_columns) > 20:
            warnings.warn(
                f"Too many features ({len(feature_columns)} > 20) for ALE to process, "
        ...
        for feature_name in feature_columns:
            ...
            if i in self.categorical_features:
                ...
                scores = self._ale_categorical(
                    data=self.data, features=features, column=i
                )
                if monte_carlo:
                    n = int(self.data.shape[0] * monte_carlo_frac)
                    if n < 10:
                        warnings.warn(
                            f"The number of samples in each Monte Carlo step is "
                            f"too small, i.e., {n} < 10. The Monte Carlo sampling is ignored."
                        )
                    else:
                        x = pd.DataFrame(
                            self.data[:, i], columns=["value"]
                        ).reset_index()
                        ...
                        for _ in range(monte_carlo_steps):
                            ...
                            for group in group_indices:
                                ...
                                indices += np.random.choice(
                                    group, n, replace=False
                                ).tolist()
                            s = self._ale_categorical(
                                data=self.data[indices], features=features, column=i
                            )
                            if len(s) == len(scores):
                ...
                if monte_carlo:
                    ...
                    if n < 10:
                        ...
                        for _ in range(monte_carlo_steps):
                            indices = np.random.choice(
                                range(self.data.shape[0]), n, replace=False
                            )
                            s = self._ale_continuous(
                                data=self.data[indices], column=i, bins=bins
                            )
                            if len(s) == len(scores):
                                sampled_scores.append(s.sort_index().values)
