...
#
"""
The explainer based Indoxrouter's Models.
"""
import os
...
class LLMExplainer(ExplainerBase):
    """
    The explainer based on Indoxrouter's Models. The input prompt consists of the feature importance scores
    and the counterfactual examples (if used). The explanations will be the text generated by
    Indoxrouter's Models.
    """
    ...
    def __init__(
        ...
        indoxrouter_model: str = "openai/gpt-4o-mini",
        indoxrouter_apikey: str = None,
        **kwargs,
    ):
        """
        ...
        :param indoxrouter_apikey: The Indoxrouter API Key.
        ...
        :param indoxrouter_model: The model type for chat completion.
        ...
        """
        super().__init__()
        self.client = Client(api_key=indoxrouter_apikey)
        self.indoxrouter_apikey = indoxrouter_apikey
        self.indoxrouter_model = indoxrouter_model
    ...
    def _generate_prompt(
        shap_explanation, mace_explanation=None, mode="classification", top_k=50
    ):
        system_prompt = (
        ...
        prompts = []
        for i, (feature, value, score) in enumerate(
            zip(
                shap_explanation["features"],
                shap_explanation["values"],
                shap_explanation["scores"],
            )
        ):
            if i < top_k:
                prompts.append('{}. "{} = {}": {:.4f}'.format(i, feature, value, score))
        context_prompt = (
            "Firstly, given the following feature importance scores in the format "
            '"<feature name>: <feature importance score>":\n\n' + "\n".join(prompts)
        )

        if mode == "classification":
            question_prompt = (
                f"Please explain why this example is classified as "
                f"label_{shap_explanation['target_label']}."
            )
        else:
            question_prompt = (
                "Please explain why this example has this predicted value."
            )
        context_prompt += (
        ...
        if (
            mace_explanation is not None
            and mace_explanation["counterfactual"] is not None
        ):
            ...
            for i, values in enumerate(cfs.values):
                ...
                if len(changed_features) > 0:
                    prompts.append(
                        "{}. If {}, then the predicted label will be label_{} instead of label_{}".format(
                            i,
                            " and ".join(changed_features),
                            cf_label,
                            shap_explanation["target_label"],
                        )
                    )
            mace_prompt = (
    ...
    @staticmethod
    def _api_call(prompt, indoxrouter_apikey, indoxrouter_model):
        # indoxrouter.api_key = (
        #     os.getenv("INDOROUTER_API_KEY")
        #     if not indoxrouter_apikey
        #     else indoxrouter_apikey
        # )
        # if not indoxrouter.api_key:
        #     raise RuntimeError("Please set your Indoxrouter API KEY.")
        client = Client(api_key=indoxrouter_apikey)

        completion = client.chat(
            model=indoxrouter_model,
            messages=[
    ...
    def explain(self, X, **kwargs) -> PlainText:
        ...
        mace_explanations = (
            self.mace_explainer.explain(X) if self.mace_explainer is not None else None
        )

        for i, e in enumerate(shap_explanations.get_explanations()):
            mace = (
                mace_explanations.get_explanations()[i]
                if mace_explanations is not None
                else None
            )
            input_prompt = self._generate_prompt(e, mace_explanation=mace)
            explanation = self._api_call(
                input_prompt, self.indoxrouter_apikey, self.indoxrouter_model
            )
