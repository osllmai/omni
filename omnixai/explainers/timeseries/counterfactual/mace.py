...
if is_torch_available():
    ...
    class _Policy(nn.Module):
        ...
        def forward(self, inputs):
            ...
            entropy = (
                torch.mean(column_prob * torch.log(column_prob))
            ) * self.entropy_weight
        ...
        def get_actions(self, column_probs, action_probs, num_actions):
            ...
            return (
                np.concatenate(batch_samples, axis=0),
        ...
        def get_probabilities(self):
            ...
            action_probs = [
                self._clip_softmax(p, dim=0).detach().cpu().numpy()
...
class MACEExplainer(ExplainerBase):
    ...
    def _candidates(self, ts_len, n_bins=8):
        ...
        # Add more diverse candidate values for better exploration
        # The lower and upper bounds
        bounds = np.stack(
            [np.quantile(ts, 0.01, axis=0), np.quantile(ts, 0.99, axis=0)]
        )

        # Add quantile-based candidates for more diversity
        quantiles = np.stack(
            [
                np.quantile(ts, 0.1, axis=0),
                np.quantile(ts, 0.25, axis=0),
                np.quantile(ts, 0.5, axis=0),
                np.quantile(ts, 0.75, axis=0),
                np.quantile(ts, 0.9, axis=0),
            ]
        )

        values = np.concatenate([bounds[:1], quantiles, values, bounds[1:]], axis=0)
    ...
    def _build_predictor(self, ts_len):
        def _predict(x: np.ndarray):
            ...
            try:
                # First try: pass all timeseries as a list
                result = self.predict_function(ts)
                ...
                try:
                    # Second try: pass individual timeseries
                    return np.array([self.predict_function(t) for t in ts]).flatten()
                except Exception as e2:
                    # Third try: pass as single timeseries if only one
                    if len(ts) == 1:
                        try:
                            result = self.predict_function(ts[0])
                            return np.array([result]).flatten()
                        except Exception as e3:
                            print(
                                f"MACEExplainer predict function error: {e1}, {e2}, {e3}"
                            )
                            # Return default values to prevent crashes
                            return np.array([0.1] * len(ts)).flatten()
                    else:
                        print(f"MACEExplainer predict function error: {e1}, {e2}")
                        return np.array([0.1] * len(ts)).flatten()
    ...
    def _build_explainer(self, ts_len):
        ...
        self.candidates, self.gamma, self.bounds, self.diff_eps = self._candidates(
            ts_len
        )

    def _reward(self, x, less_than_threshold=True, is_train=True):
        scores = self.predictor(x)
        # Fixed logic: reward should be positive when we achieve the desired outcome
        if less_than_threshold:
            # We want scores < threshold, so reward = threshold - scores
            # Positive reward when scores < threshold
            s = self.threshold - scores
        else:
            # We want scores > threshold, so reward = scores - threshold
            # Positive reward when scores > threshold
            s = scores - self.threshold
    ...
    def _optimize(
        ...
        for iteration in range(num_iterations):
            ...
            samples, batch_cols, batch_acts = policy.get_actions(
                column_prob, action_prob, num_actions=batch_size
            )
            rewards = self._reward(samples, less_than_threshold, is_train=True)
            # Use a more adaptive baseline subtraction
            if np.max(rewards) > 0:
                # If some rewards are positive, subtract median
                rewards = rewards - np.percentile(rewards, 50)
            else:
                # If all rewards are negative, subtract minimum to ensure some are positive
                rewards = rewards - np.min(rewards) + 0.01
            optimizer.zero_grad()
            loss = policy(
                (
                    torch.LongTensor(batch_cols),
                    torch.LongTensor(batch_acts),
        ...
        for column, _, prob in sorted(d, key=lambda c: c[1], reverse=True):
            vs = sorted(
                zip(policy.candidates[column], prob), key=lambda c: c[1], reverse=True
            )
            optimal.append((column, vs[0][0]))

            # Improved greedy method for picking counterfactual examples
        y = x.copy()

        original_reward = self._reward(x, less_than_threshold, is_train=False)

        # Try individual modifications first
        for i in range(len(optimal)):
            column, feature = optimal[i]
            y_temp = x.copy()
            y_temp[0, column] = feature
            reward = self._reward(y_temp, less_than_threshold, is_train=False)

            if reward > 0:
                return y_temp

        # If individual modifications don't work, try cumulative modifications

        y = x.copy()
        for i in range(len(optimal)):
            column, feature = optimal[i]
            y[0, column] = feature
            reward = self._reward(y, less_than_threshold, is_train=False)

            if reward > 0:
                return y

                # If still no success, try a more relaxed threshold (90% of target)
        relaxed_threshold = (
            self.threshold * 0.9 if less_than_threshold else self.threshold * 1.1
        )
        original_threshold = self.threshold
        self.threshold = relaxed_threshold
    ...
    def explain(self, X: Timeseries, **kwargs) -> CFExplanation:
        ...
        original_score = self.predict_function(X)
        label = int(original_score > self.threshold)
        instance = X.values.flatten()

        # Improved default parameters for better success rate
        y = self._optimize(
            x=instance,
            ...
            batch_size=kwargs.get(
                "batch_size", 100
            ),  # Larger batch for better exploration
