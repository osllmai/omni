...
class CounterfactualExplainer(ExplainerBase):
    ...
    def _build_predictor(self, ts_len):
        def _predict(x: np.ndarray):
            ...
            try:
                # First try: pass single timeseries
                ...
                try:
                    ...
                    print(f"CounterfactualExplainer predict function error: {e1}, {e2}")
    ...
    def explain(self, X: Timeseries, **kwargs) -> CFExplanation:
        ...
        x = optimizer.optimize(
            revise=kwargs.get("revise", True), verbose=kwargs.get("verbose", True)
        )
        explanations.add(
            query=Timeseries(
