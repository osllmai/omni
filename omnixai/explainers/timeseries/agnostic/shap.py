...
class ShapTimeseries(ExplainerBase):
                ...
                try:
                    ...
                    if len(ts) == 1:
                        try:
                            ...
                            print(
                                f"ShapTimeseries predict function error: {e1}, {e2}, {e3}"
                            )
                            # Return default values to prevent crashes
                            return np.array([0.1] * len(ts)).flatten()
                    else:
                        print(f"ShapTimeseries predict function error: {e1}, {e2}")
                        return np.array([0.1] * len(ts)).flatten()
    ...
    def _build_explainer(self, ts_len, num_samples=100):
        ...
        self.explainer = shap.KernelExplainer(
            model=self._build_predictor(ts_len), data=data, link="identity"
        )
    ...
    def explain(self, X: Timeseries, **kwargs) -> FeatureImportance:
        ...
        scores = pd.DataFrame(metric_shap_values, columns=X.columns, index=X.index)
