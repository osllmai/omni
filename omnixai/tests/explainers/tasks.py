...
class TabularClassification:
    ...
    def train_adult(self, num_training_samples=None):
        ...
        data = np.genfromtxt(
            os.path.join(data_dir, "adult.data"), delimiter=", ", dtype=str
        )
        ...
        if num_training_samples is not None:
            gbtree.fit(
                train[:num_training_samples], labels_train[:num_training_samples]
            )
    ...
    def train_iris(self):
        ...
        x = transformer.transform(tabular_data)
        train, test, labels_train, labels_test = (
            sklearn.model_selection.train_test_split(x, iris.target, train_size=0.80)
        )
    ...
    def train_agaricus(self):
        ...
        data_dir = os.path.join(self.base_folder, "../datasets")
        data = np.genfromtxt(
            os.path.join(data_dir, "agaricus-lepiota.data"), delimiter=",", dtype="<U20"
        )
        tabular_data = Tabular(
            data,
            feature_columns=feature_names,
            categorical_columns=feature_names[1:],
            target_column="label",
        )
...
class TabularRegression:
    ...
    def train_boston(self):
        ...
        tabular_data = Tabular(
            ...
            categorical_columns=[],  # California housing has no categorical features
        )
        transformer = TabularTransform().fit(tabular_data)
        x = transformer.transform(tabular_data)
        ...
        train, test, labels_train, labels_test = (
            sklearn.model_selection.train_test_split(
                x, california.target, train_size=0.80
            )
        )
        print("Training data shape: {}".format(train.shape))
        print("Test data shape:     {}".format(test.shape))
        ...
        print(
            "MSError when predicting the mean",
            np.mean((labels_train.mean() - labels_test) ** 2),
        )
    ...
    def train_boston_continuous(self):
        ...
        df = pd.DataFrame(
            np.concatenate(
                [california.data, california.target.reshape((-1, 1))], axis=1
            ),
            columns=list(california.feature_names) + ["target"],
        )
        # California housing has no categorical features to drop
        ...
        return Task(
            name="california_housing_continuous",
            model=rf,
            transform=transformer,
            data=tabular_data,
            train_data=transformer.invert(train),
