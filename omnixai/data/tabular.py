...
class Tabular(Data):
    ...
    def __init__(
        ...
        if isinstance(data, pd.DataFrame):
            ...
            if target_column:
                assert (
                    target_column in data
                ), f"The target column is not included in the dataframe."
        else:
            if data.ndim == 1:
    ...
    def __getitem__(self, i: Union[int, slice, list]):
        ...
        if isinstance(i, int):
            ...
            raise KeyError(
                f"Indexing a `Tabular` with key {i} of "
                f"type {type(i).__name__} is not supported."
            )
        return Tabular(
    ...
    def continuous_columns(self) -> List:
        ...
        return [
            ...
            if c not in self.categorical_cols and c != self.target_column
        ]
    ...
    def remove_target_column(self):
        ...
        if self.target_col is None:
            ...
            return Tabular(
                data=self.to_pd().drop(columns=[self.target_col]),
                categorical_columns=self.categorical_columns,
            )
    ...
    def get_continuous_medians(self) -> Dict:
        ...
        """
        return {
            c: np.median(np.abs(self.data[c].values.astype(float)))
