...
class Image(Data):
    ...
    def __init__(
        self,
        data: Union[np.ndarray, PilImage.Image] = None,
        batched: bool = False,
        ...
        if data is None:
            ...
            self.data = self._check_and_unify(
                np.array(data), batched=False, channel_last=True
            )
        else:
            raise ValueError(
                f"`data` should have type `np.ndarray` or `PIL.Image` "
        ...
        if img.ndim == 3:
            if channel_last:
                assert img.shape[2] <= 4, (
                    f"`channel_last = {channel_last}`, the last dimension of "
                    f"`data` should be the color channels."
                )
            else:
                assert img.shape[0] <= 4, (
                    f"`channel_last = {channel_last}`, the first dimension of "
                    f"`data` should be the color channels."
                )
    ...
    def to_pil(self) -> Union[PilImage.Image, List]:
        ...
        if self.shape[0] == 1:
            ...
            return [
                PilImage.fromarray(x[i].astype(np.uint8)) for i in range(self.shape[0])
            ]
