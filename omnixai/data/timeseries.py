...
class Timeseries(Data):
    ...
    def __init__(
        self, data: np.ndarray, timestamps: List = None, variable_names: List = None
    ):
        """
    ...
    def to_pd(self) -> pd.DataFrame:
        ...
        """
        return pd.DataFrame(
            self.data, columns=self.variable_names, index=self.timestamps
        )
    ...
    def reset_timestamp_index(ts, timestamp_info):
        ...
        return Timeseries(
            data, variable_names=ts.columns + ["@timestamp"], timestamps=ts.index
        )
